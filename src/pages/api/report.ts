...
import type { IConfig, IHandler, IHandlerOptions, IHandlerReader, IRestApiError } from "../../common/api"
...
const parseNewReport: ReaderTaskEither<IHandlerOptions<IReport>, IRestApiError, INewReport> = RTE.asksReaderTaskEither(flow(
	({ req }) => parseForm(req)({ multiples: true, filter: filterImages }),
	TE.mapLeft(constant<IRestApiError>({ type: "BadRequestError", errors: ["Failed to parse form data"] })),
...
function getNewReportFromForm(parsed: t.TypeOf<typeof NewReportParser>): INewReport {
	const [description] = parsed.description
	return { description }
}

const createReportHandler: IHandlerReader<IReport> = pipe(parseNewReport, RTE.chainTaskEitherK(flow(
	reportService.create,
	TE.mapLeft(constant<IRestApiError>({ type: "InternalServerError", message: "Failed to create Report" })),
)))
